...
def notification_saved(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.debug(f"Triggered Celery task for notification {event_type} event with ID {instance.id}")
    except Exception as e:
        logger.error(f"Failed to trigger Celery task for notification {event_type} event with ID {instance.id}: {e}")
...
def notification_deleted(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.debug(f"Triggered Celery task for notification deleted event with ID {instance.id}")
    except Exception as e:
        logger.error(f"Failed to trigger Celery task for notification deleted event "
                     f"with ID {instance.id}: {e}")
