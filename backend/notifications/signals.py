...
def notification_saved(sender, instance, created, **kwargs):
    ...
    logger.info(f"Triggered Celery task for notification {event_type} event with ID {instance.id}")
...
def notification_deleted(sender, instance, **kwargs):
    ...
    logger.info(f"Triggered Celery task for notification deleted event with ID {instance.id}")
