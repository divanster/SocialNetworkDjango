...
def album_created_or_updated(sender, instance, created, **kwargs):
    ...
    logger.info(f"[SIGNAL] Triggered Celery task for album {event_type} with ID {instance.pk}")
...
def album_deleted(sender, instance, **kwargs):
    ...
    # Respect the visibility setting
    if instance.visibility == VisibilityChoices.PRIVATE:
        logger.info(f"[SIGNAL] Album {instance.pk} was private. No task triggered for deletion.")
        return

    process_album_event_task.delay(str(instance.pk), 'deleted')
    logger.info(f"[SIGNAL] Triggered Celery task for album deleted with ID {instance.pk}")
...
def photo_created_or_updated(sender, instance, created, **kwargs):
    ...
    if instance.album.visibility == VisibilityChoices.PRIVATE:
        logger.info(f"[SIGNAL] Photo {instance.pk} is part of a private album. No task triggered.")
        return

    event_type = 'created' if created else 'updated'
    process_photo_event_task.delay(str(instance.pk), event_type)
    logger.info(f"[SIGNAL] Triggered Celery task for photo {event_type} with ID {instance.pk}")
...
def photo_deleted(sender, instance, **kwargs):
    ...
    # Respect the visibility setting of the associated album
    if instance.album.visibility == VisibilityChoices.PRIVATE:
        logger.info(f"[SIGNAL] Photo {instance.pk} was part of a private album. No task triggered for deletion.")
        return
