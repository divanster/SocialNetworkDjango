...
def album_saved(sender, instance, created, **kwargs):
    ...
    transaction.on_commit(
        lambda: send_album_event_to_kafka.delay(instance.id, event_type))
...
def album_deleted(sender, instance, **kwargs):
    transaction.on_commit(
        lambda: send_album_event_to_kafka.delay(instance.id, 'deleted'))
...
def photo_saved(sender, instance, created, **kwargs):
    event_type = 'created' if created else 'updated'
    transaction.on_commit(
        lambda: send_photo_event_to_kafka.delay(instance.id, event_type))
...
def photo_deleted(sender, instance, **kwargs):
    transaction.on_commit(
        lambda: send_photo_event_to_kafka.delay(instance.id, 'deleted'))
