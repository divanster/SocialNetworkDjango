...
def send_story_event_to_kafka(self, story_id, event_type):
    ...
    try:
        ...
        logger.info(
            f"Sent Kafka message for story event '{event_type}' with message: {message}")
...
def send_websocket_notification(story, event_type):
    ...
    from websocket.consumers import \
        GeneralKafkaConsumer  # Import to avoid circular dependencies
    ...
    try:
        ...
        logger.info(
            f"Real-time WebSocket notification sent for story event '{event_type}' with ID {story.id}")
    except Exception as e:
        logger.error(
            f"Error sending WebSocket notification for story ID '{story.id}': {e}")
