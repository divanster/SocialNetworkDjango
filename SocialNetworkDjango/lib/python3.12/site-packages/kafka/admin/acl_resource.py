...
from kafka_app.errors import IllegalArgumentError
...
try:
    ...
    from kafka_app.vendor.enum34 import IntEnum


class ResourceType(IntEnum):
    """Type of kafka_app resource to set ACL for
...
class ACL(ACLFilter):
    ...
    In kafka_app an ACL is a 4-tuple of (principal, host, operation, permission_type)
    ...
    Principal -> This is the identifier for the user. Depending on the authorization method used (SSL, SASL etc)
        ...
        The principal must be on the format "User:<name>" or kafka_app will treat it as invalid. It's possible to use
    ...
    Host -> This must currently be an IP address. It cannot be a range, and it cannot be a domain name.
        It can be set to "*", which is special cased in kafka_app to mean "any host"
    Operation -> Which client operation this ACL refers to. Has different meaning depending
        ...
        for a list of which combinations of resource/operation that unlocks which kafka_app APIs
...
class ResourcePattern(ResourcePatternFilter):
    ...
    Since KIP-290 (kafka_app 2.0) it's possible to set an ACL for a prefixed resource name, which
