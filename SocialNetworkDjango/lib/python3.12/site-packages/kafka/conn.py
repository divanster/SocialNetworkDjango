...
try:
    ...
    from kafka_app.vendor import selectors34 as selectors
...
import kafka_app.errors as Errors
from kafka_app.future import Future
from kafka_app.metrics.stats import Avg, Count, Max, Rate
from kafka_app.oauth.abstract import AbstractTokenProvider
from kafka_app.protocol.admin import SaslHandShakeRequest, DescribeAclsRequest_v2
from kafka_app.protocol.commit import OffsetFetchRequest
from kafka_app.protocol.offset import OffsetRequest
from kafka_app.protocol.produce import ProduceRequest
from kafka_app.protocol.metadata import MetadataRequest
from kafka_app.protocol.fetch import FetchRequest
from kafka_app.protocol.parser import KafkaProtocol
from kafka_app.protocol.types import Int32, Int8
from kafka_app.scram import ScramClient
...
class BrokerConnection(object):
    ...
    Keyword Arguments:
        ...
        metrics (kafka_app.metrics.Metrics): Optionally provide a metrics
    ...
    def _handle_sasl_handshake_response(self, future, response):
        ...
        if self.config['sasl_mechanism'] not in response.enabled_mechanisms:
            ...
            return future.failure(
                Errors.UnsupportedSaslMechanismError(
                    'kafka_app-python does not support SASL mechanism %s' %
            ...
            if not self._can_send_recv():
                ...
                try:
                    ...
                    while not client_ctx.complete:
                        # calculate an output token from kafka_app token (or None if first iteration)
                        output_token = client_ctx.step(received_token)

                        # pass output token to kafka_app, or send empty response if the security
    ...
    def close(self, error=None):
        ...
        Arguments:
            error (Exception, optional): pending in-flight-requests
                ...
                Default: kafka_app.errors.KafkaConnectionError.
        """
        if self.state is ConnectionStates.DISCONNECTED:
            return
        with self._lock:
    ...
    def check_version(self, timeout=2, strict=False, topics=[]):
        ...
        # kafka_app kills the connection when it doesn't recognize an API request
        # so we can send a test request and then follow immediately with a
        # vanilla MetadataRequest. If the server did not recognize the first
        # request, both will be failed with a ConnectionError that wraps
        # socket.error (32, 54, or 104)
        from kafka_app.protocol.admin import ApiVersionRequest, ListGroupsRequest
        from kafka_app.protocol.commit import OffsetFetchRequest, GroupCoordinatorRequest

        test_cases = [
            # All cases starting from 0.10 will be based on ApiVersionResponse
            ((0, 10), ApiVersionRequest[0]()),
            ((0, 9), ListGroupsRequest[0]()),
            ((0, 8, 2), GroupCoordinatorRequest[0]('kafka_app-python-default-group')),
            ((0, 8, 1), OffsetFetchRequest[0]('kafka_app-python-default-group', [])),
        ...
        for version, request in test_cases:
            ...
            if f.succeeded():
                if isinstance(request, ApiVersionRequest[0]):
                    # Starting from 0.10 kafka_app broker we determine version
